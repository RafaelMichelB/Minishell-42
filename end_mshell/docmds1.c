...
{
	if (fd[0] == -1)
		return (ft_putstr_fd("bash: ", 2), ft_putstr_fd(cmds[0][*i - 1].path, 2), \
		ft_putendl_fd(": No such file or directory", 2), clear_cmds(cmds[1]), free(cmds[0]), ft_free(env), exit(1));
	if (fd[0] != -1)
		close(fd[0]);
...
{
	if (access(cmds[i - 1].path, F_OK) == 0 && access(cmds[i - 1].path, W_OK) != 0)
		return (ft_putstr_fd("bash: ", 2), ft_putstr_fd(cmds[i - 1].path, 2), \
		ft_putendl_fd(": Permission denied", 2), clear_cmds(cmd), free(cmds), ft_free(env), exit(1), 0);
	return (ft_putstr_fd("bash: ", 2), ft_putstr_fd(cmds[i - 1].path, 2), \
